/**
 * ZeeZicht Booking Pro - Main Application JavaScript
 * 
 * CLEAN, EXTERNAL, PRODUCTION-READY
 */
...
function closeBookingModal() {
    ...
    if (modal) {
        ...
        window.ZZBP.currentModalStep = 1;
        showStep(1);
    }
}
...
function nextStep() {
    if (validateCurrentStep()) {
        window.ZZBP.currentModalStep++;
        showStep(window.ZZBP.currentModalStep);
    }
}

function prevStep() {
    if (window.ZZBP.currentModalStep > 1) {
        window.ZZBP.currentModalStep--;
        showStep(window.ZZBP.currentModalStep);
    }
}
...
function validateCurrentStep() {
    ...
    } else if (window.ZZBP.currentModalStep === 3) {
...
function updateGuestCount(type, increment) {
    const currentValue = window.ZZBP.modalData[type];
    let newValue;
    ...
    window.ZZBP.modalData[type] = newValue;
...
function updateConfirmation() {
    ...
    if (checkIn && checkOut) {
        ...
        if (confirmGuests) {
            confirmGuests.textContent = window.ZZBP.modalData.adults + ' volwassenen' + 
                (window.ZZBP.modalData.children > 0 ? ', ' + window.ZZBP.modalData.children + ' kinderen' : '');
        }
        if (confirmNights) confirmNights.textContent = nights;
        
        const subtotal = nights * window.ZZBP.modalData.basePrice;
...
function submitBooking() {
    const formData = {
        ...
        adults: window.ZZBP.modalData.adults,
        children: window.ZZBP.modalData.children,
...
function initModalCalendar() {
    ...
    // Calendar initialization logic will be added here
}
...
document.addEventListener('DOMContentLoaded', function() {
    console.log('ZZBP: DOM ready, initializing...');
    ...
    console.log('ZZBP: Initialization complete');
});
